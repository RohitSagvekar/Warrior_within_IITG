...
void detectNode(){ //This set of functions detect nodes at which the decision making will occur
  if(statusSensor1 == 1 or statusSensor5 == 1){
    ...
    else if(statusSensor3==1 and prevSensor1==1 and prevSensor5 !=1){ //can be removed
        tLeftFlag = 1; 
    }
    else if(statusSensor3==1 and prevSensor1 != 1 and prevSensor5==1){ //can be removed
        tRightFlag = 1;
...
if(mapFlag==1){ // THIS IS THE MAPPING LOOP. 
...
if(moveFlag==1){ // THIS IS THE MOVING LOOP 
  ...
  //Append terminus as last target location.
    ...
    else if(currentBed!=0){ //find out last common node, then negate the entries after that. 
        int i=0;
        int k=0;
    ...
    if(xFlag==1 or tLeftFlag==1 or tRightFlag==1 or tStraightFlag==1){//This part makes bot take turn decisions based on tempAddress matrix
      if(tempAddress[currentTurn]=='L'){ //repeat for 4 instances
        turnLeft();
      }
